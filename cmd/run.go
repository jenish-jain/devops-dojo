...
func RunCmd(infoFile string) *cobra.Command {
	return &cobra.Command{
		...
		Long: `example next pending exercise : devopsdojo run next
example specific exercise : devopsdojo run variables1`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				...
				color.Yellow("If you feel stuck, ask a hint by executing `devopsdojo hint %s`", result.Exercise.Name)
			} else {
